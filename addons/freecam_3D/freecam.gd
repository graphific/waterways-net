...
##
## Usage: Run your game, press <TAB> to toggle freecam mode, then use arrow keys to fly around.
## Controls: Arrow Keys = move, PageUp/PageDown = up/down, Mouse = look around, Mouse Wheel = speed
##
...
func _add_keybindings() -> void:
	...
	# Changed to arrow keys to avoid conflicts with game controls
	if "__debug_camera_forward" not in actions: _add_key_input_action("__debug_camera_forward", KEY_UP)
	if "__debug_camera_back" 	not in actions: _add_key_input_action("__debug_camera_back", KEY_DOWN)
	if "__debug_camera_left" 	not in actions: _add_key_input_action("__debug_camera_left", KEY_LEFT)
	if "__debug_camera_right" 	not in actions: _add_key_input_action("__debug_camera_right", KEY_RIGHT)
	if "__debug_camera_up" 		not in actions: _add_key_input_action("__debug_camera_up", KEY_PAGEUP)
	if "__debug_camera_down" 	not in actions: _add_key_input_action("__debug_camera_down", KEY_PAGEDOWN)
